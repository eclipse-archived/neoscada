...
public class ProfileActionBarContributor extends EditingDomainActionBarContributor implements ISelectionChangedListener
{
    ...
    protected IAction refreshViewerAction = new Action ( WorldEditorPlugin.INSTANCE.getString ( "_UI_RefreshViewer_menu_item" )) {
        ...
        {
            return ProfileActionBarContributor.this.activeEditorPart instanceof IViewerProvider;
        }

        @Override
        public void run ()
        {
            if ( ProfileActionBarContributor.this.activeEditorPart instanceof IViewerProvider )
            {
                final Viewer viewer = ( (IViewerProvider)ProfileActionBarContributor.this.activeEditorPart ).getViewer ();
                if ( viewer != null )
                {
                    viewer.refresh ( );
                }
    ...
    public void contributeToMenu ( final IMenuManager menuManager )
    {
        ...
        final IMenuManager submenuManager = new MenuManager ( WorldEditorPlugin.INSTANCE.getString ( "_UI_ProfileEditor_menu" ), "org.eclipse.scada.configuration.world.osgi.profileMenuID" ); //$NON-NLS-1$ //$NON-NLS-2$
