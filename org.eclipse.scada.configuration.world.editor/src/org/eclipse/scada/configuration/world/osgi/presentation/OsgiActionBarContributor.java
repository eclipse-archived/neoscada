...
public class OsgiActionBarContributor extends EditingDomainActionBarContributor implements ISelectionChangedListener
{
    ...
    protected IAction refreshViewerAction = new Action ( WorldEditorPlugin.INSTANCE.getString ( "_UI_RefreshViewer_menu_item" )) {
        ...
        {
            return OsgiActionBarContributor.this.activeEditorPart instanceof IViewerProvider;
        }

        @Override
        public void run ()
        {
            if ( OsgiActionBarContributor.this.activeEditorPart instanceof IViewerProvider )
            {
                final Viewer viewer = ( (IViewerProvider)OsgiActionBarContributor.this.activeEditorPart ).getViewer ();
                if ( viewer != null )
                {
                    viewer.refresh ( );
                }
    ...
    public void contributeToMenu ( final IMenuManager menuManager )
    {
        ...
        final IMenuManager submenuManager = new MenuManager ( WorldEditorPlugin.INSTANCE.getString ( "_UI_OsgiEditor_menu" ), "org.eclipse.scada.configuration.world.osgiMenuID" ); //$NON-NLS-1$ //$NON-NLS-2$
