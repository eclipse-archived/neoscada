...
public class WorldActionBarContributor extends EditingDomainActionBarContributor implements ISelectionChangedListener
{
    ...
    protected IAction refreshViewerAction = new Action ( WorldEditorPlugin.INSTANCE.getString ( "_UI_RefreshViewer_menu_item" )) {
        ...
        {
            return WorldActionBarContributor.this.activeEditorPart instanceof IViewerProvider;
        }

        @Override
        public void run ()
        {
            if ( WorldActionBarContributor.this.activeEditorPart instanceof IViewerProvider )
            {
                final Viewer viewer = ( (IViewerProvider)WorldActionBarContributor.this.activeEditorPart ).getViewer ();
                if ( viewer != null )
                {
                    viewer.refresh ( );
                }
    ...
    public void contributeToMenu ( final IMenuManager menuManager )
    {
        ...
        final IMenuManager submenuManager = new MenuManager ( WorldEditorPlugin.INSTANCE.getString ( "_UI_WorldEditor_menu" ), "org.eclipse.scada.configuration.worldMenuID" ); //$NON-NLS-1$ //$NON-NLS-2$
