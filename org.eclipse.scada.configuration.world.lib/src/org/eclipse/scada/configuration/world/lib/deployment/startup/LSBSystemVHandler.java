...
public class LSBSystemVHandler implements StartupHandler
{
    ...
    {
        File initFile;
        Helper.createFile ( initFile = new File ( packageFolder, "src/etc/init.d/scada.driver." + driverName ), LSBSystemVHandler.class.getResourceAsStream ( "templates/lsb_sysv/driver.service.sh" ), replacements, monitor, ALT_PATTERN ); //$NON-NLS-1$
    ...
    {
        {
            ...
            Helper.createFile ( file, LSBSystemVHandler.class.getResourceAsStream ( "templates/lsb_sysv/p2.service.sh" ), replacements, monitor, ALT_PATTERN ); //$NON-NLS-1$
            file.setExecutable ( true );
        }
        {
            final File file = new File ( packageFolder, "src/usr/bin/scada.app." + appName + ".launcher" ); //$NON-NLS-1$  //$NON-NLS-2$
            Helper.createFile ( file, LSBSystemVHandler.class.getResourceAsStream ( "templates/redhat_sysv/p2.launcher.sh" ), replacements, monitor, ALT_PATTERN ); //$NON-NLS-1$
            file.setExecutable ( true );
        }
    }
